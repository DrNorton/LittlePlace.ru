...
EventManager.prototype.getMyOwnEvents = function (userId,params, onSuccess, onError) {
    var self = this;
...
function AddEvent(name,eventTime,latitude,longitude,ownerId,address,description,onSuccess, onError, sqlManager) {
    ...
    var query = format("Insert into [littleplace_db].[dbo].[Event] (Name,Created,EventTime,Latitude,Longitude,OwnerId,Address,Description) Values ('{0}',{1},{2},{3},{4},{5},'{6}','{7}')  ", name, "GETDATE()", "GETDATE()", latitude, longitude, ownerId, address, description);
